 ...
 * 各自实现此接口，Pipeline 中的 MotStage 仅依赖此抽象。
 *
 * 使用方式：
 * @code
 *   kflee::mot::MotConfig cfg;
 *   cfg.frame_rate = 30;
 *   auto tracker = kflee::mot::MotRegistry::instance()
 *       .create("byte_track", cfg);
 *
 *   // 逐帧调用
...
public:
    ...
    /** 允许移动 */
    Mot(Mot&&) noexcept = default;
    Mot& operator=(Mot&&) noexcept = default;
